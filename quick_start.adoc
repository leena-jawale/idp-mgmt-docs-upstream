...
OpenID identity provider configuration provides identities for users authenticating using OpenID credentials. You can configure Red Hat Single Sign-On as an OpenID Connect identity provider.

=== AuthRealm custom resource for OpenID
An example of an AuthRealm custom resource configured for the OpenID identity provider is provided below. Make note of the `identityProviders` field and the configuration of the `opeinid` identity provider under it:
[source,yaml]
----
apiVersion: identityconfig.identitatem.io/v1alpha1
kind: AuthRealm
...
- `openID` contains the OpenID specific configurations:
  * `clientID` contains the client ID of OAuth.
  * `clientSecret` contains a reference to a secret object containing the client secret of OAuth. 
  - `claims` maps attributes on the user entry.
    * `preferredUsername` is the list of attributes whose values should be used as the preferred username. Optional. If unspecified, the preferred username is determined from the value of the sub claim
    * `name` is the list of attributes whose values should be used as the display name. Optional. If unspecified, no display name is set for the identity
    * `email` is the list of attributes whose values should be used as the email address. Optional. If unspecified, no email is set for the identity
